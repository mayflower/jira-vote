...
use Ma27\Jira\IssueVoteBundle\Util\Client\Jira as JiraClient;
use Ma27\Jira\IssueVoteBundle\Util\Generator\OAuthCredentialUrlGeneratorInterface;
...
class OAuthSecurityAccessProvider implements RestProviderInterface
{
    ...
    /**
     * Executes an request with the oauth plugin
     *
     * @param string   $uri     Target uri on the server
     * @param string   $method  Method of the request
     * @param string[] $headers List of headers
     * @param boolean  $raw     Is this variable true, the content will be returned raw, if not, the content will be decoded as json
     *
     * @return mixed
     *
     * @api
     */
    public function executeRequest($uri, $method = 'post', array $headers = array(), $raw = false)
    {
    ...
    protected function fetchTokensFromJira($uri, $token = null, $tokenSecret = null)
    {
        ...
        $tokenString = $this->client->post($uri, ['Content-Type: application/json'])->send()->getBody(true);
